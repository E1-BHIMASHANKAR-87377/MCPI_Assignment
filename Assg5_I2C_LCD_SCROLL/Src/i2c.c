...
void I2C_Start(void) {
    ...
    while(!(I2C1->SR1 & I2C_SR1_SB));       //sb 1: Start condition generated.
}
...
void I2C_Stop(void) {
    ...
    while(!(I2C1->SR2 & I2C_SR2_BUSY));      //Set by hardware on detection of SDA or SCL low
}
...
uint16_t I2C_RecvData(void) {
	...
	uint16_t val = I2C1->DR;       //when we write data on data start transmitt
	return val;
}
...
uint16_t I2C_RecvDataNAck(void) {
	I2C1->CR1 &= ~(I2C_CR1_ACK | I2C_CR1_POS);
	// wait until receive buffer is not empty
	while (!(I2C1->SR1 & I2C_SR1_RXNE));    //rxne 1 until register has valid data
	// read content and clear flags
...
int I2C_IsDeviceReady(uint8_t slaveAddr) {
	...
	// SR1 -- poll for addr is transferred
    while (!(I2C1->SR1 & I2C_SR1_ADDR));
    // dummy read to clear flags addr flage
    (void)I2C1->SR1;
    (void)I2C1->SR2;
    return 1;
}
